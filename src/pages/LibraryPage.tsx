...
export function LibraryPage({ fixedType }: { fixedType?: ItemType }) {
  ...
  const visibleNotebooks = useMemo(() => notebooks.filter((board) => !board.parentId).filter((board) => {
  ...
  return (
    <div className="page-shell section-shell library-page">
      ...
      {loading ? <LoadingState label={text('Opening the library…', 'Bibliothek wird geöffnet…')} /> : visible.length || visibleNotebooks.length ? (
        <div className="content-grid library-grid">{visibleNotebooks.map((board) => { const children = notebooks.filter((candidate) => candidate.parentId === board.id); const pages = board.pages.length + children.reduce((total, child) => total + child.pages.length, 0); return <article key={board.id} className={`content-card type-project notebook-project-card cover-${board.pages[0]?.coverStyle || 'blossom'}`} style={{ '--cover-image': board.coverImage ? `url("${board.coverImage}")` : 'none' } as React.CSSProperties}><Link to={`/notebooks?board=${encodeURIComponent(board.id)}`} className="content-card-link"><div className="card-visual"><NotebookTabs size={38} strokeWidth={1.5} /><span className="type-pill"><FolderKanban size={13} />{text('Notebook project', 'Lernheft-Projekt')}</span></div><div className="card-content"><div className="card-kicker"><span>{children.length + 1} {text('notebooks', 'Lernhefte')}</span><span>·</span><span>{pages} {text('pages', 'Seiten')}</span><span>·</span><span>{board.viewCount || 0} {text('views', 'Aufrufe')}</span></div><h3>{board.title}</h3><p>{text('One complete study notebook with its related subboards and learning material.', 'Ein vollständiges Lernheft mit zugehörigen Unterboards und Lernmaterial.')}</p><div className="card-footer"><span className="read-link">{text('Open book', 'Lernheft öffnen')} <ArrowUpRight size={15} /></span></div></div></Link></article> })}{visible.map((item) => <ContentCard key={item.id} item={item} />)}</div>
      ) : (
