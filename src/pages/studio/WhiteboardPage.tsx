    ...
    <div className="whiteboard-layout">
      ...
      {board && page && <section className="whiteboard-workspace">
        ...
        <div className="whiteboard-toolbar" role="toolbar" aria-label="Drawing tools">
          ...
          <label className="ruling-size">Spacing <input type="range" min="10" max="140" step="1" value={page.rulingSize || 40} onChange={(event) => updatePage({ rulingSize: Number(event.target.value) })} aria-label="Paper line spacing slider" /><input className="precise-number" type="number" min="10" max="140" step="1" value={page.rulingSize || 40} onChange={(event) => updatePage({ rulingSize: Math.max(10, Math.min(140, Number(event.target.value) || 10)) })} aria-label="Exact paper line spacing" /><span>px</span></label>
