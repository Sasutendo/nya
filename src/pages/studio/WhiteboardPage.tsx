...
export function WhiteboardPage() {
  ...
  useEffect(() => {
    ...
    adminApi.whiteboards().then(async ({ boards: loaded }) => {
      ...
      const firstPage: WhiteboardPageData = { id: newId('page'), name: 'Page 1', background: 'grid', paperSize: 'a4', orientation: 'portrait', accentColour: '#bd5d87', coverStyle: 'blossom', strokes: [] }
    ...
    <div className="whiteboard-layout">
      ...
      {board && page && <section className="whiteboard-workspace">
        <div className="whiteboard-topbar">
          ...
          <select className="board-cover-select" value={board.pages[0]?.coverStyle || 'blossom'} onChange={(event) => { const coverStyle = event.target.value as WhiteboardPageData['coverStyle']; updateBoard({ pages: board.pages.map((item) => ({ ...item, coverStyle })) }) }} aria-label="Notebook cover"><option value="blossom">🌸 Blossom cover</option><option value="clinical">✚ Clinical cover</option><option value="night">✦ Night study cover</option><option value="strawberry">🍓 Strawberry cover</option><option value="minimal">Minimal cover</option></select>
