...
import { BookOpen, ExternalLink, Globe2, Layers3, NotebookTabs, Search } from 'lucide-react'
import { Link, useSearchParams } from 'react-router-dom'
import { getPublicItems, getPublicStudyCards, getPublicWhiteboards } from '../lib/api'
import { EmptyState, LoadingState } from '../components/Feedback'
import { useLanguage } from '../lib/i18n'
import type { ContentItem, StudyCard, WhiteboardBoard } from '../types'
...
export function SearchPage() {
  ...
  useEffect(() => { Promise.all([getPublicItems(), getPublicWhiteboards(), getPublicStudyCards()]).then(([content, notebooks, flashcards]) => { setItems(content); setBoards(notebooks); setCards(flashcards) }).finally(() => setLoading(false)) }, [])
  useEffect(() => {
  ...
  const local = useMemo(() => {
    ...
    const flashcards = cards.filter((card) => `${card.category} ${card.question} ${card.answer}`.toLowerCase().includes(needle)).map((card) => ({ id: card.id, title: card.question || text('Handwritten flashcard', 'Handgeschriebene Karteikarte'), excerpt: `${text('Flashcard deck', 'Karteikarten-Stapel')}: ${card.category}`, to: '/flashcards', kind: 'flashcard' }))
    return [...flashcards, ...notebooks, ...content]
  }, [boards, cards, items, params, text])
